...
% Matt: I would use SNV, rather than SNP, since SNP is more commonly used to refer to germline variants.  Or just use "somatic mutation calling pipeline"
...
%
% Matt:
%  a.  LOH can be real or an amplification artifact
%  b.  isn't penetrance more about phenotype?  i would use polymorphic (for partially penetrant) and fixed (for fully penetrant)
%  c.  i think the main reason for lack of multi-sample sequencing is that TCGA and other have used pathology specimens (i.e. there is only one per patient) out of convenience
...
% Matt:
%  a.  the first citation looks weird in the PDF.  it says (Network, 2008), which I think should be (TCGA Network, 2008)
